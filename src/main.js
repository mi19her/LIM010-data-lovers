...
ordenar.addEventListener('click', (event) => {
    let ordenSeleccionado=event.target.value;
    if(ordenSeleccionado==='ascendente'){
        console.log(window.worldbank.formulaOrdenar(years));
    }else{
        console.log(window.worldbank.formulaOrdenar(years).reverse());
    }
...
promedio.addEventListener('click', (event) => {
    ...
    let resultadoPromedio = worldbank.formulaPromedio(datoPromedio);
